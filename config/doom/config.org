...
However, I store my journal entries in a directory encrypted using [[https://github.com/rfjakob/gocryptfs][gocryptfs]] that auto-unmounts on idle. Here, I advise ~org-roam-dailies--capture~, which is the function called whenever visiting or capturing to a daily file. This advice checks if the directory is currently mounted; if not, then we should of course mount it.
#+begin_src elisp
