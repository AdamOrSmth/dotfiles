...
+ INVESTIGATE :: An external source or information to look into later. In its own lonely sequence because it's the only keyword I want to log a note when moving away from, and which also moves to =COMPLETE=. Would otherwise override note logging with only time instead, since the final state's logging takes precedence over the starting state.
