...
#+end_src
* =gpt.el=
Look, I wrote my own package! Except it's not really a package cause it doesn't have its own repository or anything, it's just here. Time to sell my soul to OpenAI as well. I'm loading the package using the black, magical box of =use-package=; I add my Doom configuration directory to the load path and also provide a list of commands so that the file gets loaded lazily.
