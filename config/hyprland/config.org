...
+ s-{j/k} :: Switch focus to the next/previous window.
#+begin_src conf
...
+ s-{h/l} :: Switch focus to the leftward/rightward monitor.
+ s-S-{h/l} :: Move the focused window to the leftward/rightward monitor.
#+begin_src conf
bind = SUPER,H,focusmonitor,l
bind = SUPER,L,focusmonitor,r
bind = SUPERSHIFT,H,movewindow,mon:l
bind = SUPERSHIFT,L,movewindow,mon:r
#+end_src
...
Resizing works the same way as swapping windows; a sub-map using Vi-like keybindings. Shift does smaller adjustments.
