...

{
  config = lib.mkIf (!config.ad.isServer) {
    # Community overlay for newer versions of Emacs
    nixpkgs.overlays = [
    ...
    services.emacs = {
      ...
      defaultEditor = true;
      package = with pkgs;
      # Install Emacs 28 with native compilation support
      # and the `alpha-background` patch, and include
      # the `Vterm` package as it doesn't compile on NixOS
        ((emacsPackagesFor (emacsNativeComp.overrideAttrs (oldAttrs: {
          patches = [
            (fetchurl {
              url =
